...
for (i in 5){
  ...
  for (i in topSNP){
    ...
    if (data[1,6] >= 0.8){#提取共定位成功的SNP数据
      locus <- rbind(locus, input)
    }
  }
}
command_locus <- paste0('C:/SMR/graph/locuszoom/', exposure, '_', outcome, '_CAD.txt')
write.table(locus, command_locus, row.names = FALSE, sep = "\t", quote = FALSE)#保存共定位成功的SNP数据
...
command_locus <- paste0('C:/SMR/graph/locuszoom/', exposure, '_', outcome, '_MS.txt')
write.table(locus, command_locus, row.names = FALSE, sep = "\t", quote = FALSE)

...
data1 <- data1[data1$chrom == 17,]
data1 <- data1[data1$pval <= 0.1,]
data2 <- read.table('C:/SMR/coloc/phenotype/CAD_coloc.txt', sep = '\t', header = T)
data2 <- data2[data2$CHROM == 17,]
data2 <- data2[data2$p <= 0.1,]

gene <- read.table('C:/SMR/coloc/phenotype/coloc_bind/region_gene/lip_region14.CSV',sep = ',', header = T)
gene1 <- gene[23,]
gene1$y <- -1
...
p<-ggplot()+
  ...
  scale_x_continuous(breaks = seq(46900000,47900000,by=200000), limits = c(46900000,47900000))+
 ...
 #geom_segment(aes(x = Start, y = y+0.4, xend = Start, yend = y-0.4), data = gene1)+
  ...
  labs(title = 'Chromosome 17 (hyperlipidemia)',
       x = 'posiotion',
       y = '-log10(P)')+
  theme_minimal()+
...
ggsave('C:/SMR/graph/locuszoom/lip_blood_np_gene.pdf', p, width =8, height = 8, dpi = 300)

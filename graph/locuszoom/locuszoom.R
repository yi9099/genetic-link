library(ggplot2)
data <- read.table('C:/SMR/graph/locuszoom/diabetes.txt',sep = '\t', header = T)
...
P <- ggplot(data)+
  geom_point(aes(x = POS, y = -log10(p)), color = '#62B197')+
  geom_point(aes(x = pos, y = -log10(pval)), color = '#F0A780')+

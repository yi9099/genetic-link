...
data1 <- data1[data1$chrom == 6,]
data1 <- data1[data1$pval <= 0.1,]
data2 <- read.table('C:/SMR/coloc/phenotype/CAD_coloc.txt', sep = '\t', header = T)
data2 <- data2[data2$CHROM == 6,]
data2 <- data2[data2$p <= 0.1,]

gene <- read.table('C:/SMR/coloc/phenotype/coloc_bind/region_gene/ten_region42.CSV',sep = ',', header = T)
set <- c(15,24)
gene1 <- gene[set,]
gene1$y <- seq(-1,-2)
...
p<-ggplot()+
  ...
  scale_x_continuous(breaks = seq(34000000,35000000,by=200000), limits = c(34000000,35000000))+
...
ggsave('C:/SMR/graph/locuszoom/ten_blood_IS.pdf', p, width =8, height = 8, dpi = 300)
